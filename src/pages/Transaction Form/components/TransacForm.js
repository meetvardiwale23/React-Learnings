...
const TransacForm = () => {
  ...
  const [getOnchangeErrors, setOnchangeErros] = useState({
    transactionDate: "",
    monthYear: "",
    transactionType: "",
    fromAccount: "",
  ...
  const submit = (e) => {
    ...
    let onChangeError = { ...getOnchangeErrors };
    ...
    Object.keys(validateFormValues).map((key, index) => {
      if (validateFormValues[key] === "") {
        ...
        validateFormValues["fromAccount"] === validateFormValues["toAccount"]
      ) {
        sendError = {
          ...sendError,
    ...
    const checkOnchangeError = Object.values(onChangeError).filter(
      (item) => item !== ""
    );
    // console.log("checkOnCHANGEerror", checkOnchangeError);
    const checkError = Object.values(sendError).filter((item) => item !== "");

    if (checkError.length === 0 && checkOnchangeError.length === 0) {
      if (localStorage.getItem("FormData")) {
        ...
        if (addData) {
          if (state) {
            const getindex = addData.findIndex((indexValue) => {
              return indexValue.id === state;
            });
            addData[getindex] = getFormValues;
                    ...
                    >
                      ...
                      <div className="d-flex justify-content-between align-items-center pb-2">
                        ...
                        <div>
                          ...
                          {getOnchangeErrors.notes ? (<label>
                            {getOnchangeErrors.notes}
                          </label>): ("")}
                        </div>
                      </div>
                      {/* submit */}
